        ...
        textTransform(_ctx, src) {
            ...
            if (opts.wikilinks) {
                ...
                src = src.replace(wikilinkRegex, (value, ...capture) => {
                    const [rawFp, rawHeader, rawAlias]: (string | undefined)[] =
                        capture;
                    ...
                    if (opts.wikilinks) {
                        replacements.push([
                            ...
                            (value: string, ...capture: string[]) => {
                                ...
                                const alias: string | undefined = rawAlias
                                    ?.slice(1)
                                    .trim();
                                ...
                                if (value.startsWith("!")) {
                                    ...
                                    if (
                                        ...
                                        const match =
                                            wikilinkImageEmbedRegex.exec(
                                                alias ?? "",
                                            );
                                        const alt = match?.groups?.alt ?? "";
                                        const width =
                                            match?.groups?.width ?? "auto";
                                        const height =
                                            match?.groups?.height ?? "auto";
                                        return {
                                            type: "image",
                                            url,
                                            data: {
                    ...
                    return (tree: HtmlRoot, file) => {
                        ...
                        visit(tree, "element", (node, index, parent) => {
                            if (blockTagTypes.has(node.tagName)) {
                                const nextChild = parent?.children.at(
                                    index! + 2,
                                ) as Element;
                                if (nextChild && nextChild.tagName === "p") {
                                    const text = nextChild.children.at(
                                        0,
                                    ) as Literal;
                                    if (
                                        text &&
                                ...
                                if (
                                    ...
                                    const matches =
                                        last.value.match(blockReferenceRegex);
                                    if (matches && matches.length >= 1) {
                    ...
                    return (tree: HtmlRoot) => {
                        visit(tree, "element", (node) => {
                            if (
                                ...
                                const match =
                                    node.properties.src.match(ytLinkRegex);
                                const videoId =
                                    match && match[2].length == 11
                                        ? match[2]
                                        : null;
                                const playlistId =
                                    node.properties.src.match(
                                        ytPlaylistLinkRegex,
                                    )?.[1];
                                if (videoId) {
