...
function toggleExplorer(this: HTMLElement) {
    ...
    nearestExplorer.setAttribute(
        "aria-expanded",
        nearestExplorer.getAttribute("aria-expanded") === "true"
            ? "false"
            : "true",
    );
...
function createFolderNode(
    ...
    const clone = template.content.cloneNode(true) as DocumentFragment;
    const li = clone.querySelector("li") as HTMLLIElement;
    const folderContainer = li.querySelector(
        ".folder-container",
    ) as HTMLElement;
    ...
    if (opts.folderClickBehavior === "link") {
        ...
        const span = titleContainer.querySelector(
            ".folder-title",
        ) as HTMLElement;
    ...
    const isCollapsed =
        currentExplorerState.find((item) => item.path === folderPath)
            ?.collapsed ?? opts.folderDefaultState === "collapsed";
    ...
    for (const explorer of allExplorers) {
        ...
        const opts: ParsedOptions = {
            ...
            filterFn: new Function(
                "return " + (dataFns.filterFn || "undefined"),
            )(),
