    ...
    if (argv.serve) {
        ...
        const server = http.createServer(async (req, res) => {
            ...
            const serve = async () => {
                ...
                console.log(
                    statusString +
                        styleText("grey", ` ${argv.baseDir}${req.url}`),
                );
                release();
            };
...
export async function handleSync(argv) {
    ...
    if (argv.commit) {
        ...
        const commitMessage =
            argv.message ?? `Quartz sync: ${currentTimestamp}`;
    ...
    if (argv.pull) {
        ...
        try {
            ...
            console.log(
                styleText(
                    "red",
                    "An error occurred above while pulling updates.",
                ),
            );
            await popContentFolder(contentFolder);
            return;
    ...
    if (argv.push) {
        ...
        const res = spawnSync(
            "git",
            ["push", "-uf", ORIGIN_NAME, currentBranch],
            {
                stdio: "inherit",
            },
        );
